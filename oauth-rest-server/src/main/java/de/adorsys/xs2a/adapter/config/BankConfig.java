...
public class BankConfig {
    ...
    public String getClientId(String bank){
        isBankSupported(bank);
        return bankConfig.get(bank).get(CLIENT_ID);
    }

    public String getRedirectUri(String bank) {
        isBankSupported(bank);
        return bankConfig.get(bank).get(REDIRECT_URI);
    }

    private void isBankSupported(String bank) {
        if (!bankConfig.containsKey(bank)) {
            throw new IllegalArgumentException(bank+" is not supported by psd2-oauth service");
        }
    }
}
